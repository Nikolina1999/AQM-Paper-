...
```{r function to delet stuff because of RAM}

# Function to delete several objects at once
# we want to keep environment clean at all times because we have large data to process 

delete_objects <- function(..., envir = .GlobalEnv) {
  objects <- list(...)
  for (obj in objects) {
    if (exists(obj, envir = envir)) {
      ...
      cat("Object", obj, "not found in the environment.\n")
    }
  }
}
...
delete_objects("cses_imd", "cses", "selected_vars",
               "countries", "years", "cy_interest",
               "country", "year")
...
# Keep the environment clean
delete_objects("selected_dataset")
```
...
# Keep the environment clean
delete_objects("DEU_2002_d", "DEU_2005_d", "NLD_1998_d",
               "PRT_2015_d", "SWE_2006_d", "USA_2004_d",
               "USA_2004_d", "ESP_2008_d", "GBR_2015_d",
               "DEU_2002", "DEU_2005", "NLD_1998", 
               "PRT_2015", "SWE_2006", "USA_2004", 
               "ESP_2008", "GBR_2015")
...
delete_objects("AP_six_spe", "AP_FRA_scores", "AP_Full_scores", 
               "AP_party_df", "AP_all_scores",
               "combined_df", "FRA_selected_d", "Full_selected_d")
```
...
merged_data <- merged_data %>%
  ...
  select(year, country, party_dislike, party_like, cntryyr, to_pfeml, to_prior_seats, WAP) %>%
  na.omit()
...
# check that to_pfeml is no NAs
sum(is.na(merged_data$to_pfeml))


# Need log1p transformation for WAP 
hist((merged_data$WAP),breaks = 50, main= "WAP")
qqnorm(merged_data$WAP)
qqline(merged_data$WAP, col = "red")
...
# Keep envrionment clean
delete_objects("AP_party_long", "multilevel_data", "selected_d")
...
pp_check(truncated_model_with_priors)
png(pp_check_full.png)

# Cross-validation
loo_result <- loo(truncated_model_with_priors)
print(loo_result)

```


```{r, correct plot predictions WAP and to_pfeml}
...
```{r, residual plot of truncated model full}

# Function to process data in batches for residuals
batch_predict_residuals <- function(model, data, batch_size = 1000) {
  n <- nrow(data)
  results <- list()
  ...
  for (i in seq(1, n, by = batch_size)) {
    ...
    batch_data <- data[i:min(i + batch_size - 1, n), ]
    predictions <- posterior_predict(model, newdata = batch_data, draws = 100)
    results[[length(results) + 1]] <- colMeans(predictions)  # Use mean of draws for residual calculation
  }
  
  unlist(results)
}

# Generate predictions for the original data in batches
predicted_values <- batch_predict_residuals(truncated_model_with_priors, merged_data, batch_size = 1000)

# Calculate residuals
observed_values <- merged_data$log_WAP
...
residuals_data <- data.frame(
  ...
  Residuals = residuals,
  to_pfeml = merged_data$to_pfeml
)

# Plot residuals against predicted values
residuals_plot <- ggplot(residuals_data, aes(x = Predicted, y = Residuals)) +
...
# Save the residual plot as a .jpg file
ggsave("Residuals_vs_Predicted.jpg", plot = residuals_plot, width = 8, height = 6, dpi = 300)

# Plot residuals against to_pfeml values
residuals_to_pfeml_plot <- ggplot(residuals_data, aes(x = to_pfeml, y = Residuals)) +
  geom_point(alpha = 0.3) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  labs(title = "Residuals vs Female Delegates Proportion",
       x = "Female Delegates Proportion",
       y = "Residuals") +
  theme_classic()

# Save the residual plot as a .jpg file
ggsave("Residuals_vs_to_pfeml.jpg", plot = residuals_to_pfeml_plot, width = 8, height = 6, dpi = 300)

```

# Robustness check 

```{r, cross validation with 1/3 and 2/3 od data}
set.seed(123)

# Sample 33.33% of data within each group
sampled_data_1_3 <- merged_data %>%
  group_by(cntryyr) %>%
  sample_frac(0.3333) %>%
  ungroup()
...
# Use four cores parallel to enhence computataion process 
plan(multisession, workers = 4)

# Train the model on the sampled data (33.33% of each group)
truncated_model_1_3 <- brm(
  bf(log_WAP | trunc(lb = log1p(0), ub = log1p(4)) ~ to_pfeml + (1 + to_pfeml | cntryyr)),
  data = sampled_data_1_3,
  family = gaussian(),
  chains = 4,
  iter = 4000,
  warmup = 1000,
  cores = 4,
  save_model = "trained_model.rds",
  prior = priors
)
